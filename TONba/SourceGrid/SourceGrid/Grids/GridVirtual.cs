...
namespace SourceGrid
{
	...
	public abstract class GridVirtual : CustomScrollControl
	{
		...
		protected virtual bool GetScrollPositionToShowCell(Position position, bool partial, out Point newScrollPosition)
		{
			...
			else
			{
				...
				else
				{
                    ...
                    if (CustomScrollPosition.X + ActualFixedColumns > position.Column)
                    {
                    ...
                    else
                    {
                        ...
                        x = GetLeftVisibleColFromRightCol(position.Column) - ActualFixedColumns;
                        //x = ExtendTopRowByHiddenRows(y);
                    }
                }
		...
		protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
		{
            ...
            else if(keyData == Keys.Left || keyData == Keys.Right || keyData == Keys.Down || keyData == Keys.Up)
            {
                KeyEventArgs args = new KeyEventArgs(keyData);
                OnKeyDown(args);
                if (args.Handled)
...
//		http://www.microsoft.com/middleeast/msdn/mirror.aspx
        ...
        int GetLeftVisibleColFromRightCol(int RightCol)
        {
            int displayWidth = DisplayRectangle.Width;
            int currentWidth = 0;
            ...
            int c = 0;
            for (c = RightCol; c >= ActualFixedColumns; c--)
            {
                currentWidth += Columns.GetWidth(c);
                ...
                if (currentWidth > displayWidth)
                {
                    if (c == RightCol) return c; else return c + 1;
                }
