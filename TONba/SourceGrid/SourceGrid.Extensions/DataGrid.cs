...
namespace SourceGrid
{
	...
	{
        public DataTable OriginalDataTable = null;
        public I_IT.Database db = null;
        ...
        {
            int rNow = Math.Max(Selection.ActivePosition.Row, 0);
            int cNow = Math.Max(Selection.ActivePosition.Column, 0);
            int rTo = r + 1; //+1 for header
            int cTo = c;

            Selection.MoveActiveCell(rTo - rNow, cTo - cNow);
        }
        ...
        {
            get { return ((BoundDataView)mBoundList).DataTable; }
            set
            {
                Unbind();
                
                mBoundList = new DevAge.ComponentModel.BoundDataView(value.DefaultView);
                if (mBoundList != null)
                    Bind();

                OriginalDataTable = value.Copy();
            }
        }
