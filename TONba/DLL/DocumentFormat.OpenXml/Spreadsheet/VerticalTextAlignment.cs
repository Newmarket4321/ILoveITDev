...
namespace DocumentFormat.OpenXml.Spreadsheet
{
  ...
  public class VerticalTextAlignment : OpenXmlLeafElement
  {
    ...
    public EnumValue<VerticalAlignmentRunValues> Val
    {
      get
      {
        return (EnumValue<VerticalAlignmentRunValues>) this.Attributes[0];
      }
      set
      {
    ...
    internal override OpenXmlSimpleType AttributeFactory(byte namespaceId, string name)
    {
      if ((int) namespaceId == 0 && "val" == name)
        return (OpenXmlSimpleType) new EnumValue<VerticalAlignmentRunValues>();
      return base.AttributeFactory(namespaceId, name);
    }

    public override OpenXmlElement CloneNode(bool deep)
    {
      return (OpenXmlElement) this.CloneImp<VerticalTextAlignment>(deep);
    }
  }
}
