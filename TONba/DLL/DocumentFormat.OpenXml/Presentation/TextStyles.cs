// Decompiled with JetBrains decompiler
// Type: DocumentFormat.OpenXml.Presentation.TextStyles
// Assembly: DocumentFormat.OpenXml, Version=2.5.5631.0, Culture=neutral, PublicKeyToken=31bf3856ad364e35
// MVID: A7316BF1-EDFD-42B4-84FD-A64CEF0DCF01
// Assembly location: C:\Users\graeme smyth\Documents\Visual Studio 2015\Projects\I❤IT\I❤IT\bin\Debug\DocumentFormat.OpenXml.dll

using System.CodeDom.Compiler;
using System.Collections.Generic;

namespace DocumentFormat.OpenXml.Presentation
{
  [ChildElementInfo(typeof (BodyStyle))]
  [ChildElementInfo(typeof (OtherStyle))]
  [GeneratedCode("DomGen", "2.0")]
  [ChildElementInfo(typeof (TitleStyle))]
  [ChildElementInfo(typeof (ExtensionList))]
  public class TextStyles : OpenXmlCompositeElement
  {
    private static readonly string[] eleTagNames = new string[4]
    {
      "titleStyle",
      "bodyStyle",
      "otherStyle",
      "extLst"
    };
    private static readonly byte[] eleNamespaceIds = new byte[4]
    {
      (byte) 24,
      (byte) 24,
      (byte) 24,
      (byte) 24
    };
    private const string tagName = "txStyles";
    private const byte tagNsId = 24;
    internal const int ElementTypeIdConst = 12436;

    public override string LocalName
    {
      get
      {
        return "txStyles";
      }
    }

    internal override byte NamespaceId
    {
      get
      {
        return 24;
      }
    }

    internal override int ElementTypeId
    {
      get
      {
        return 12436;
      }
    }

    internal override bool IsInVersion(FileFormatVersions version)
    {
      return ((FileFormatVersions.Office2007 | FileFormatVersions.Office2010 | FileFormatVersions.Office2013) & version) > FileFormatVersions.None;
    }

    public TextStyles()
    {
    }

    public TextStyles(IEnumerable<OpenXmlElement> childElements)
      : base(childElements)
    {
    }

    public TextStyles(params OpenXmlElement[] childElements)
      : base(childElements)
    {
    }

    public TextStyles(string outerXml)
      : base(outerXml)
    {
    }

    internal override OpenXmlElement ElementFactory(byte namespaceId, string name)
    {
      if (24 == (int) namespaceId && "titleStyle" == name)
        return (OpenXmlElement) new TitleStyle();
      if (24 == (int) namespaceId && "bodyStyle" == name)
        return (OpenXmlElement) new BodyStyle();
      if (24 == (int) namespaceId && "otherStyle" == name)
        return (OpenXmlElement) new OtherStyle();
      if (24 == (int) namespaceId && "extLst" == name)
        return (OpenXmlElement) new ExtensionList();
      return (OpenXmlElement) null;
    }

    internal override string[] ElementTagNames
    {
      get
      {
        return TextStyles.eleTagNames;
      }
    }

    internal override byte[] ElementNamespaceIds
    {
      get
      {
        return TextStyles.eleNamespaceIds;
      }
    }

    internal override OpenXmlCompositeType OpenXmlCompositeType
    {
      get
      {
        return OpenXmlCompositeType.OneSequence;
      }
    }

    public TitleStyle TitleStyle
    {
      get
      {
        return this.GetElement<TitleStyle>(0);
      }
      set
      {
        this.SetElement<TitleStyle>(0, value);
      }
    }

    public BodyStyle BodyStyle
    {
      get
      {
        return this.GetElement<BodyStyle>(1);
      }
      set
      {
        this.SetElement<BodyStyle>(1, value);
      }
    }

    public OtherStyle OtherStyle
    {
      get
      {
        return this.GetElement<OtherStyle>(2);
      }
      set
      {
        this.SetElement<OtherStyle>(2, value);
      }
    }

    public ExtensionList ExtensionList
    {
      get
      {
        return this.GetElement<ExtensionList>(3);
      }
      set
      {
        this.SetElement<ExtensionList>(3, value);
      }
    }

    public override OpenXmlElement CloneNode(bool deep)
    {
      return (OpenXmlElement) this.CloneImp<TextStyles>(deep);
    }
  }
}
