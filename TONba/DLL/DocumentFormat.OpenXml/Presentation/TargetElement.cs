// Decompiled with JetBrains decompiler
// Type: DocumentFormat.OpenXml.Presentation.TargetElement
// Assembly: DocumentFormat.OpenXml, Version=2.5.5631.0, Culture=neutral, PublicKeyToken=31bf3856ad364e35
// MVID: A7316BF1-EDFD-42B4-84FD-A64CEF0DCF01
// Assembly location: C:\Users\graeme smyth\Documents\Visual Studio 2015\Projects\I❤IT\I❤IT\bin\Debug\DocumentFormat.OpenXml.dll

using DocumentFormat.OpenXml.Office2010.PowerPoint;
using System.CodeDom.Compiler;
using System.Collections.Generic;

namespace DocumentFormat.OpenXml.Presentation
{
  [ChildElementInfo(typeof (ShapeTarget))]
  [ChildElementInfo(typeof (BookmarkTarget), FileFormatVersions.Office2010 | FileFormatVersions.Office2013)]
  [ChildElementInfo(typeof (SoundTarget))]
  [ChildElementInfo(typeof (SlideTarget))]
  [ChildElementInfo(typeof (InkTarget))]
  [GeneratedCode("DomGen", "2.0")]
  public class TargetElement : OpenXmlCompositeElement
  {
    private static readonly string[] eleTagNames = new string[5]
    {
      "sldTgt",
      "sndTgt",
      "spTgt",
      "inkTgt",
      "bmkTgt"
    };
    private static readonly byte[] eleNamespaceIds = new byte[5]
    {
      (byte) 24,
      (byte) 24,
      (byte) 24,
      (byte) 24,
      (byte) 49
    };
    private const string tagName = "tgtEl";
    private const byte tagNsId = 24;
    internal const int ElementTypeIdConst = 12255;

    public override string LocalName
    {
      get
      {
        return "tgtEl";
      }
    }

    internal override byte NamespaceId
    {
      get
      {
        return 24;
      }
    }

    internal override int ElementTypeId
    {
      get
      {
        return 12255;
      }
    }

    internal override bool IsInVersion(FileFormatVersions version)
    {
      return ((FileFormatVersions.Office2007 | FileFormatVersions.Office2010 | FileFormatVersions.Office2013) & version) > FileFormatVersions.None;
    }

    public TargetElement()
    {
    }

    public TargetElement(IEnumerable<OpenXmlElement> childElements)
      : base(childElements)
    {
    }

    public TargetElement(params OpenXmlElement[] childElements)
      : base(childElements)
    {
    }

    public TargetElement(string outerXml)
      : base(outerXml)
    {
    }

    internal override OpenXmlElement ElementFactory(byte namespaceId, string name)
    {
      if (24 == (int) namespaceId && "sldTgt" == name)
        return (OpenXmlElement) new SlideTarget();
      if (24 == (int) namespaceId && "sndTgt" == name)
        return (OpenXmlElement) new SoundTarget();
      if (24 == (int) namespaceId && "spTgt" == name)
        return (OpenXmlElement) new ShapeTarget();
      if (24 == (int) namespaceId && "inkTgt" == name)
        return (OpenXmlElement) new InkTarget();
      if (49 == (int) namespaceId && "bmkTgt" == name)
        return (OpenXmlElement) new BookmarkTarget();
      return (OpenXmlElement) null;
    }

    internal override string[] ElementTagNames
    {
      get
      {
        return TargetElement.eleTagNames;
      }
    }

    internal override byte[] ElementNamespaceIds
    {
      get
      {
        return TargetElement.eleNamespaceIds;
      }
    }

    internal override OpenXmlCompositeType OpenXmlCompositeType
    {
      get
      {
        return OpenXmlCompositeType.OneChoice;
      }
    }

    public SlideTarget SlideTarget
    {
      get
      {
        return this.GetElement<SlideTarget>(0);
      }
      set
      {
        this.SetElement<SlideTarget>(0, value);
      }
    }

    public SoundTarget SoundTarget
    {
      get
      {
        return this.GetElement<SoundTarget>(1);
      }
      set
      {
        this.SetElement<SoundTarget>(1, value);
      }
    }

    public ShapeTarget ShapeTarget
    {
      get
      {
        return this.GetElement<ShapeTarget>(2);
      }
      set
      {
        this.SetElement<ShapeTarget>(2, value);
      }
    }

    public InkTarget InkTarget
    {
      get
      {
        return this.GetElement<InkTarget>(3);
      }
      set
      {
        this.SetElement<InkTarget>(3, value);
      }
    }

    public BookmarkTarget BookmarkTarget
    {
      get
      {
        return this.GetElement<BookmarkTarget>(4);
      }
      set
      {
        this.SetElement<BookmarkTarget>(4, value);
      }
    }

    public override OpenXmlElement CloneNode(bool deep)
    {
      return (OpenXmlElement) this.CloneImp<TargetElement>(deep);
    }
  }
}
