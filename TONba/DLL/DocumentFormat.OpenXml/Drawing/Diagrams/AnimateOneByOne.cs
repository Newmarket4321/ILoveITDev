...
namespace DocumentFormat.OpenXml.Drawing.Diagrams
{
  ...
  public class AnimateOneByOne : OpenXmlLeafElement
  {
    ...
    public EnumValue<AnimateOneByOneValues> Val
    {
      get
      {
        return (EnumValue<AnimateOneByOneValues>) this.Attributes[0];
      }
      set
      {
    ...
    internal override OpenXmlSimpleType AttributeFactory(byte namespaceId, string name)
    {
      if ((int) namespaceId == 0 && "val" == name)
        return (OpenXmlSimpleType) new EnumValue<AnimateOneByOneValues>();
      return base.AttributeFactory(namespaceId, name);
    }

    public override OpenXmlElement CloneNode(bool deep)
    {
      return (OpenXmlElement) this.CloneImp<AnimateOneByOne>(deep);
    }
  }
}
