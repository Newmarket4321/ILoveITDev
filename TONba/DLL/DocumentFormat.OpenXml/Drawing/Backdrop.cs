// Decompiled with JetBrains decompiler
// Type: DocumentFormat.OpenXml.Drawing.Backdrop
// Assembly: DocumentFormat.OpenXml, Version=2.5.5631.0, Culture=neutral, PublicKeyToken=31bf3856ad364e35
// MVID: A7316BF1-EDFD-42B4-84FD-A64CEF0DCF01
// Assembly location: C:\Users\graeme smyth\Documents\Visual Studio 2015\Projects\I❤IT\I❤IT\bin\Debug\DocumentFormat.OpenXml.dll

using System.CodeDom.Compiler;
using System.Collections.Generic;

namespace DocumentFormat.OpenXml.Drawing
{
  [ChildElementInfo(typeof (Normal))]
  [GeneratedCode("DomGen", "2.0")]
  [ChildElementInfo(typeof (Anchor))]
  [ChildElementInfo(typeof (UpVector))]
  [ChildElementInfo(typeof (ExtensionList))]
  public class Backdrop : OpenXmlCompositeElement
  {
    private static readonly string[] eleTagNames = new string[4]
    {
      "anchor",
      "norm",
      "up",
      "extLst"
    };
    private static readonly byte[] eleNamespaceIds = new byte[4]
    {
      (byte) 10,
      (byte) 10,
      (byte) 10,
      (byte) 10
    };
    private const string tagName = "backdrop";
    private const byte tagNsId = 10;
    internal const int ElementTypeIdConst = 10195;

    public override string LocalName
    {
      get
      {
        return "backdrop";
      }
    }

    internal override byte NamespaceId
    {
      get
      {
        return 10;
      }
    }

    internal override int ElementTypeId
    {
      get
      {
        return 10195;
      }
    }

    internal override bool IsInVersion(FileFormatVersions version)
    {
      return ((FileFormatVersions.Office2007 | FileFormatVersions.Office2010 | FileFormatVersions.Office2013) & version) > FileFormatVersions.None;
    }

    public Backdrop()
    {
    }

    public Backdrop(IEnumerable<OpenXmlElement> childElements)
      : base(childElements)
    {
    }

    public Backdrop(params OpenXmlElement[] childElements)
      : base(childElements)
    {
    }

    public Backdrop(string outerXml)
      : base(outerXml)
    {
    }

    internal override OpenXmlElement ElementFactory(byte namespaceId, string name)
    {
      if (10 == (int) namespaceId && "anchor" == name)
        return (OpenXmlElement) new Anchor();
      if (10 == (int) namespaceId && "norm" == name)
        return (OpenXmlElement) new Normal();
      if (10 == (int) namespaceId && "up" == name)
        return (OpenXmlElement) new UpVector();
      if (10 == (int) namespaceId && "extLst" == name)
        return (OpenXmlElement) new ExtensionList();
      return (OpenXmlElement) null;
    }

    internal override string[] ElementTagNames
    {
      get
      {
        return Backdrop.eleTagNames;
      }
    }

    internal override byte[] ElementNamespaceIds
    {
      get
      {
        return Backdrop.eleNamespaceIds;
      }
    }

    internal override OpenXmlCompositeType OpenXmlCompositeType
    {
      get
      {
        return OpenXmlCompositeType.OneSequence;
      }
    }

    public Anchor Anchor
    {
      get
      {
        return this.GetElement<Anchor>(0);
      }
      set
      {
        this.SetElement<Anchor>(0, value);
      }
    }

    public Normal Normal
    {
      get
      {
        return this.GetElement<Normal>(1);
      }
      set
      {
        this.SetElement<Normal>(1, value);
      }
    }

    public UpVector UpVector
    {
      get
      {
        return this.GetElement<UpVector>(2);
      }
      set
      {
        this.SetElement<UpVector>(2, value);
      }
    }

    public ExtensionList ExtensionList
    {
      get
      {
        return this.GetElement<ExtensionList>(3);
      }
      set
      {
        this.SetElement<ExtensionList>(3, value);
      }
    }

    public override OpenXmlElement CloneNode(bool deep)
    {
      return (OpenXmlElement) this.CloneImp<Backdrop>(deep);
    }
  }
}
