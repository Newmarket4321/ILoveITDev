// Decompiled with JetBrains decompiler
// Type: DocumentFormat.OpenXml.Drawing.ObjectDefaults
// Assembly: DocumentFormat.OpenXml, Version=2.5.5631.0, Culture=neutral, PublicKeyToken=31bf3856ad364e35
// MVID: A7316BF1-EDFD-42B4-84FD-A64CEF0DCF01
// Assembly location: C:\Users\graeme smyth\Documents\Visual Studio 2015\Projects\I❤IT\I❤IT\bin\Debug\DocumentFormat.OpenXml.dll

using System.CodeDom.Compiler;
using System.Collections.Generic;

namespace DocumentFormat.OpenXml.Drawing
{
  [GeneratedCode("DomGen", "2.0")]
  [ChildElementInfo(typeof (ShapeDefault))]
  [ChildElementInfo(typeof (LineDefault))]
  [ChildElementInfo(typeof (TextDefault))]
  [ChildElementInfo(typeof (ExtensionList))]
  public class ObjectDefaults : OpenXmlCompositeElement
  {
    private static readonly string[] eleTagNames = new string[4]
    {
      "spDef",
      "lnDef",
      "txDef",
      "extLst"
    };
    private static readonly byte[] eleNamespaceIds = new byte[4]
    {
      (byte) 10,
      (byte) 10,
      (byte) 10,
      (byte) 10
    };
    private const string tagName = "objectDefaults";
    private const byte tagNsId = 10;
    internal const int ElementTypeIdConst = 10337;

    public override string LocalName
    {
      get
      {
        return "objectDefaults";
      }
    }

    internal override byte NamespaceId
    {
      get
      {
        return 10;
      }
    }

    internal override int ElementTypeId
    {
      get
      {
        return 10337;
      }
    }

    internal override bool IsInVersion(FileFormatVersions version)
    {
      return ((FileFormatVersions.Office2007 | FileFormatVersions.Office2010 | FileFormatVersions.Office2013) & version) > FileFormatVersions.None;
    }

    public ObjectDefaults()
    {
    }

    public ObjectDefaults(IEnumerable<OpenXmlElement> childElements)
      : base(childElements)
    {
    }

    public ObjectDefaults(params OpenXmlElement[] childElements)
      : base(childElements)
    {
    }

    public ObjectDefaults(string outerXml)
      : base(outerXml)
    {
    }

    internal override OpenXmlElement ElementFactory(byte namespaceId, string name)
    {
      if (10 == (int) namespaceId && "spDef" == name)
        return (OpenXmlElement) new ShapeDefault();
      if (10 == (int) namespaceId && "lnDef" == name)
        return (OpenXmlElement) new LineDefault();
      if (10 == (int) namespaceId && "txDef" == name)
        return (OpenXmlElement) new TextDefault();
      if (10 == (int) namespaceId && "extLst" == name)
        return (OpenXmlElement) new ExtensionList();
      return (OpenXmlElement) null;
    }

    internal override string[] ElementTagNames
    {
      get
      {
        return ObjectDefaults.eleTagNames;
      }
    }

    internal override byte[] ElementNamespaceIds
    {
      get
      {
        return ObjectDefaults.eleNamespaceIds;
      }
    }

    internal override OpenXmlCompositeType OpenXmlCompositeType
    {
      get
      {
        return OpenXmlCompositeType.OneSequence;
      }
    }

    public ShapeDefault ShapeDefault
    {
      get
      {
        return this.GetElement<ShapeDefault>(0);
      }
      set
      {
        this.SetElement<ShapeDefault>(0, value);
      }
    }

    public LineDefault LineDefault
    {
      get
      {
        return this.GetElement<LineDefault>(1);
      }
      set
      {
        this.SetElement<LineDefault>(1, value);
      }
    }

    public TextDefault TextDefault
    {
      get
      {
        return this.GetElement<TextDefault>(2);
      }
      set
      {
        this.SetElement<TextDefault>(2, value);
      }
    }

    public ExtensionList ExtensionList
    {
      get
      {
        return this.GetElement<ExtensionList>(3);
      }
      set
      {
        this.SetElement<ExtensionList>(3, value);
      }
    }

    public override OpenXmlElement CloneNode(bool deep)
    {
      return (OpenXmlElement) this.CloneImp<ObjectDefaults>(deep);
    }
  }
}
