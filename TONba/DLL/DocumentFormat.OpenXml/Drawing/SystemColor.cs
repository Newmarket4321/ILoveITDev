...
namespace DocumentFormat.OpenXml.Drawing
{
  ...
  public class SystemColor : OpenXmlCompositeElement
  {
    ...
    public EnumValue<SystemColorValues> Val
    {
      get
      {
        return (EnumValue<SystemColorValues>) this.Attributes[0];
      }
      set
      {
    ...
    internal override OpenXmlSimpleType AttributeFactory(byte namespaceId, string name)
    {
      if ((int) namespaceId == 0 && "val" == name)
        return (OpenXmlSimpleType) new EnumValue<SystemColorValues>();
      if ((int) namespaceId == 0 && "lastClr" == name)
    ...
    {
      return (OpenXmlElement) this.CloneImp<SystemColor>(deep);
    }
  }
}
