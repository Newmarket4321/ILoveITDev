...
namespace DocumentFormat.OpenXml.Wordprocessing
{
  ...
  public class HorizontalMerge : OpenXmlLeafElement
  {
    ...
    public EnumValue<MergedCellValues> Val
    {
      get
      {
        return (EnumValue<MergedCellValues>) this.Attributes[0];
      }
      set
      {
    ...
    internal override OpenXmlSimpleType AttributeFactory(byte namespaceId, string name)
    {
      if (23 == (int) namespaceId && "val" == name)
        return (OpenXmlSimpleType) new EnumValue<MergedCellValues>();
      return base.AttributeFactory(namespaceId, name);
    }

    public override OpenXmlElement CloneNode(bool deep)
    {
      return (OpenXmlElement) this.CloneImp<HorizontalMerge>(deep);
    }
  }
}
