...
namespace I_IT
{
    public partial class ARReport : Form
    {
        public ARReport()
        {
            InitializeComponent();
...
sum(TAX_DUE+PEN_DUE+TAX_ADJ+PEN_ADJ+OTHER_ADJ+OTHER_DUE-TAX_PAID-PEN_PAID-OTHER_PAID) AS ""Tax Due Balance""

FROM VAILTECH.TX_MAINHT

GROUP BY VAILTECH.TX_MAINHT.ROLL_NO

HAVING sum(TAX_DUE+PEN_DUE+TAX_ADJ+PEN_ADJ+OTHER_ADJ+OTHER_DUE-TAX_PAID-PEN_PAID-OTHER_PAID) <> 0
