...
namespace I_IT
{
    ...
    public partial class RicherTextBox : UserControl
    {
        ...
        {
            get { return rtbDocument.ReadOnly; }
            set { rtbDocument.ReadOnly = value; }
        }
        ...
        public RichTextBoxPrintCtrl getRTFBox()
        {
            return rtbDocument;
        }
        ...
        private void tsbtnOpen_Click(object sender, EventArgs e)
        {
            using (OpenFileDialog dlg = new OpenFileDialog())
            {
                dlg.Filter = "Word Documents|*.doc;*.docx|Rich Text Format|*.rtf";
                dlg.FilterIndex = 0;
                if (dlg.ShowDialog() == DialogResult.OK)
                {
                    try
        ...
        public delegate void RTFEventHandler();
        public delegate void RTFLinkEventHandler(LinkClickedEventArgs e);
        public delegate void RTFMouseEventHandler(MouseEventArgs e);
        public event RTFEventHandler RTFBoxTextChanged;
        public event RTFEventHandler RTFBoxPrintRequested;
        public event RTFEventHandler RTFBoxRightClicked;
        public event RTFMouseEventHandler RTFBoxRightMouseUp;
        public event RTFLinkEventHandler RTFBoxLinkClicked;
        ...
        private void rtbDocument_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.Control && e.KeyCode == Keys.F)
                tsbtnFind_Click(this, null);
            if (e.Control && e.KeyCode == Keys.P)
                RTFBoxPrintRequested();
            if (e.Control && e.KeyCode == Keys.B)
                tsbtnBIU_Click(tsbtnBold, null);
            if (e.Control && e.KeyCode == Keys.I)
            {
                tsbtnBIU_Click(tsbtnItalic, null);
                e.SuppressKeyPress = true; //Don't insert a tab character, the default Ctrl+I command.
            }
            if (e.Control && e.KeyCode == Keys.U)
                tsbtnBIU_Click(tsbtnUnderline, null);
        }

        private void rtbDocument_Click(object sender, EventArgs e)
        {
