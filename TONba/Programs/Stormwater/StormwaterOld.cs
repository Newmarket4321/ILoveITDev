...
namespace I_IT
{
    public partial class StormwaterOld : Form
    {
        double lowArea = 0;
        double mediumArea = 0;
        double highArea = 0;
        ...
        public StormwaterOld()
        {
            ...
            //townOwned = Vailtech.Run("select * from TX_ROLL where OWNER_1 = 'NEWMARKET TOWN'");
        ...
        private void start()
        {
            try
            {
                ...
                DataTable assess = Vailtech.Run("select roll_no, class, sum(ASSESSMENT) as ASSESSMENT from TX_ROLL_ASSESS where ASS_YEAR = " + year + " group by roll_no, class");
                townOwned = Vailtech.Run("select * from TX_ROLL where OWNER_1 = 'NEWMARKET TOWN'");
                schoolboardOwned = Vailtech.Run("select * from TX_ROLL where OWNER_1 not like '%COLLEGE%' and UNIT_CLASS = 'EDN'");
                totalAssessment = Vailtech.Run("select roll_no, sum(assessment) from TX_ROLL_ASSESS where ass_year = " + year + " group by roll_no");
                ...
                Vailtech.Run("delete from stormwater where year = " + year);

                if(putOnline.Checked)
                    SQL.Run("use stormwater; delete from results");

                Vailtech.Run("delete from tx_roll_locals where LOCAL_ID = 'SWCHARGE' or LOCAL_ID = 'SWCREDIT'");
                ...
                Invoke((MethodInvoker)delegate {
                    ...
                    setEnabled(true);
                    Core.export("Stormwater", Vailtech.Run("select * from stormwater where year = " + year));
                });
            }
            catch(Exception e2)
            {
        ...
        {
            Vailtech ora = new Vailtech("select * from stormwatervacant where roll_no=@rollno");
        ...
        {
            Vailtech ora = new Vailtech("select * from stormwatervacant where roll_no=@rollno and parking_lot='Y'");
            ora.AddParameter("@rollno", rollNo);
            DataTable dt = ora.Run();
        ...
        public void calculateAreas(DataTable dt)
        {
            ...
            //DataTable dt = Vailtech.Run("select roll_no from TX_ROLL where roll_no like '%0000' and BILLABLE='Y'");
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                try
                {
                    streetNo = Vailtech.Run("select street_no from tx_roll where roll_no = '" + rollNo + "'").Rows[0][0].ToString();
                    streetName = Vailtech.Run("select street_name from tx_roll where roll_no = '" + rollNo + "'").Rows[0][0].ToString();
                }
                catch
                {
        ...
        public void process(string rollNo, int runoffCode, string className)
        {
            ...
            Vailtech ora = new Vailtech("insert into stormwater values (@ROLLNO, @YEAR, @AREA, @CLASS, @RUNOFF, @RUNOFFRATE, @CHARGE, @REBATE)");
            ora.AddParameter("@ROLLNO", rollNo);
            ...
            if(putOnBills.Checked)
            {
                ...
                {
                    ora = new Vailtech("insert into tx_roll_locals values (@ROLLNO, @ACTIVITYNO, @LOCALID, @YEARLYCHARGE, @UNITS)");
                    ora.AddParameter("@ROLLNO", rollNo);
            ...
            {
                Vailtech temp = new Vailtech("select block_1_total from tx_billprint where roll_no = @ROLLNO and tax_year = @YEAR and interim_fin_sup = @CODE");
                ...
                sql.AddParameter("@ROLLNO", rollNo);
                sql.AddParameter("@STREETNO", Vailtech.Run("select street_no from tx_roll where roll_no = " + rollNo).Rows[0][0].ToString());
                sql.AddParameter("@STREETNAME", Vailtech.Run("select street_name from tx_roll where roll_no = " + rollNo).Rows[0][0].ToString());
        ...
        {
            Vailtech ora = new Vailtech("select * from stormwatercredits where roll_no = @rollno and expiry >= @TODAY");
            ora.AddParameter("@rollno", rollNo);
...
and ASS_YEAR = " + year + @"");
        ...
        {
            Vailtech ora = new Vailtech("select LEGAL1 from TX_ROLL where ROLL_NO=@ROLLNO and (LEGAL1 like '%CONDO%' or LEGAL1 like '%YRSCP%' or LEGAL1 like '%YRCP%')");
            ora.AddParameter("@ROLLNO", rollNo);
            DataTable dt = ora.Run();
...
and TONGIS.VIEW_ASMNT_PARCEL.roll_no = '" + rollNo + "'");
            ...
            DataTable temp = Vailtech.Run("select * from vailtech.tx_prop_data where roll_no=@ROLL and property_code in (702, 703)", rollNo);
...
and ' ' || LEGAL5 not like '%PKG%'");
        ...
        {
            Core.export("Stormwater Credits", Vailtech.Run("select * from stormwatercredits"));
