...
group by employeeid, username");
            ...
            //DateTime today = new DateTime(2017, 2, 20);
...
group by employeeid");
        ...
        public static void createStat(string empID, DateTime today)
        {
            ...
            //int timesheetID = int.Parse(SQL.Run("SELECT MAX(TIMECARDDETAILID) FROM TimeSheetDetail").Rows[0][0].ToString()) + 1;
...
u.employeeid = @EMPID");
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
                if (dt.Rows[i]["department"].ToString().Trim() == "IT")
                    return true;
...
select YAEST from CRPDTA.F060116 where YAAN8 = @EMPNO");
        ...
        private static bool isHolidayForParttime(DateTime date)
        {
            bool returnable = false;
            ...
            try
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    if (DateTime.Parse(dt.Rows[i][0].ToString()) == date && !dt.Rows[i]["description"].ToString().ToUpper().Contains("EASTER") && !dt.Rows[i]["description"].ToString().ToUpper().Contains("CIVIC"))
                    {
                        returnable = true;
                    }
                }
